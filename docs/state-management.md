...
- **RTK Query** handles all server cache. One slice per resource: `playersApi`, `teamsApi`, `talentsApi`, `sponsorsApi`, `matchesApi`/`bracketsApi`, `themesApi`, `assetsApi`, `videosApi`, `projectsApi` (tournament CRUD), `rundownsApi`, `rundownOverlaysApi`, `broadcastApi`. Cache tags mirror the etalon: `User`, `Rundowns`, `Rundown`, `RundownOverlays`, `Midi`, `Seating`, and one per entity — every tag scoped by the tournament (`project_id`).
