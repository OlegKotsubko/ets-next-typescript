...
ETS uses **better-auth** with email + password. There is no public sign-up; the operator team creates user accounts out of band with `scripts/create-user.ts`.
...
The session returned by `auth.api.getSession` contains `user.id`, `user.email`, `user.name`, and `session.expiresAt`. `rundowns.owner_id` references `user.id`; use it for audit columns later.
