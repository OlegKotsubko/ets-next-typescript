...
ETS ("Esports Titles System") serves broadcast graphics (lower-thirds, scoreboards, player cards, sponsor bugs, timers, brackets) to OBS/vMix. An operator **enters a tournament**, builds a **rundown of overlays**, and drives them live onto **displays**; each display's `/preview` and `/air` page renders a transparent 1920×1080 canvas fed by Server-Sent Events.
...
- `/preview/[displayUuid]`, `/air/[displayUuid]` (**public** — OBS/vMix browser sources; display UUIDs are unguessable share-link tokens). Transparent canvas; SSE-driven set of overlays.
- `/api/projects/[projectId]/...` (protected REST) · `GET /api/broadcast/[displayUuid]/stream?channel=preview|air` (public SSE, **Node**).
