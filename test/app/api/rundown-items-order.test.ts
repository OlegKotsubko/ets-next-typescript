...
function currentIds(ids: string[]) {
  dbMock.select.mockReturnValue(chain(ids.map((id) => ({ id }))))
}
...
it('200 and writes index positions', async () => {
  ...
  // first select → current ids; second select → final read-back
  dbMock.select
    .mockReturnValueOnce(chain([{ id: A }, { id: B }]))
    .mockReturnValue(chain([{ id: B, position: 0 }, { id: A, position: 1 }]))
